...
export default function DriverPortal() {
  ...
  useEffect(() => {
    ...
    const driverId = localStorage.getItem('driver_id') || 'TRK-101';
      ...
      <header className="bg-zinc-900 border-b border-zinc-800 px-6 py-4 flex items-center justify-between sticky top-0 z-50">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-indigo-500/20 text-indigo-300 font-bold border border-indigo-500/30">
                {driver?.vehicleId || 'TN-01-TR-8841'}
              </span>
            </div>
            <span className="text-xs text-zinc-400">Driver: {driver?.name || 'Murugan'} • {driver?.vehicleType || 'Heavy Trailer'}</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            {driver?.status === 'online' ? 'TRUCK ONLINE' : 'TRUCK OFFLINE'}
          </button>
        </div>
      </header>
      ...
      <main className="flex-1 p-6 space-y-6 max-w-5xl w-full mx-auto">
        ...
        {activeOrder ? (
          ...
          <div className="bg-zinc-900/40 border border-zinc-800/80 rounded-3xl p-12 text-center text-zinc-500 space-y-3">
            ...
            <h3 className="text-base font-bold text-zinc-300">No Active Heavy Cargo Assigned</h3>
            <p className="text-xs text-zinc-500 max-w-sm mx-auto">
              Remain online in the Tamil Nadu freight corridor. You will receive real-time dispatch alerts when shippers confirm quotation bills.
            </p>
          </div>
        )}
